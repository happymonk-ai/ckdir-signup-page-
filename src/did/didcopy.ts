...
export default class DID{
    ...
    async encodeDID(signaturePublicKey: Uint8Array, encryptionPublicKey:Uint8Array): Promise<string> {
		const bytes = new Uint8Array((encryptionPublicKey.length + signaturePublicKey.length)+ 2)
		bytes[0] = 0x12 // set to ckdr encryption std. must be defined; 
		bytes[1] = 0x01
		bytes.set(encryptionPublicKey, 2)
		bytes.set(signaturePublicKey, 3)
...
export const generateDID = async(type:string)=>{
...
async function keyToDidDoc(pubKeyBytes, fingerprint) {
...
export class DID implements IDID {
	...
	async did_createJWS(params: CreateJWSParams): Promise<GeneralJWS> {
		return new Promise<GeneralJWS>(async (resolve, reject) => {
			...
			resolve( { jws: this.toGeneralJWS(jws) })
		})
	}
